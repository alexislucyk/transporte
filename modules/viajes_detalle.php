...
$puedeEditarGastosAdelantos = $estadoEditable || ($fromCobranzas && in_array($viaje['estado'], ['descargado', 'facturado', 'cobrado']));
    ...
    // ─── REGISTRAR DESCARGA ───────────────────────────
    if ($estadoEditable && $action === 'descargar') {
        $id = $viaje_id;
            ...
            try {
                ...
                if (!empty($viaje['chofer_porcentaje']) && $viaje['chofer_porcentaje'] > 0 && !empty($viaje['chofer_id'])) {
                    ...
                    if ($sobrante > 0) {
                        ...
                        $stmt_sobrante->execute([$viaje['chofer_id'], date('Y-m-d'), $sobrante, "Sobrante Adelanto {$detalle_ref}"]);
                    }
                }
...
$estado_badge = match($viaje['estado']) {
    'en_viaje'   => '<span class="badge" style="background:#f39c12; color:#fff; font-size:0.9rem; padding:6px 14px;">Constants 🚛 En Viaje</span>',
...
<div id="viajes_detalle-page" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px; flex-wrap: wrap; gap: 10px;">
    ...
    <div style="display:flex; gap:8px; flex-wrap:wrap;">
        ...
        <?php else: ?>
        <span class="badge" style="background:#95a5a6; color:#fff; font-size:0.9rem; padding:6px 14px;">
            🔎 Consulta (Continuar desde Cobranzas)
        </span>
        <?php endif; ?>
    </div>
</div>
    ...
    <div style="display:grid; grid-template-columns: 1fr 1fr 1fr; gap:14px;">
            ...
            <div style="display:flex; align-items:center; gap:12px; background:#fff; border-radius:8px; padding:10px; border:1px solid #e0e0e0;">
                ...
                <div style="flex:1;">
                    ...
                    <div style="font-weight:bold; font-size:0.95rem; color:#333;">
                        <?= htmlspecialchars($viaje['chofer_nombre'] ?? 'Sin asignar') ?>
                    </div>
                </div>
        ...
        <div style="background:linear-gradient(135deg, #fff3e0 0%, #f5f5f5 100%); border-radius:12px; padding:16px; border:1px solid #ffe0b2;">
            ...
            <div style="margin-bottom:8px; background:#fff; border-radius:8px; padding:8px 10px; border:1px solid #e0e0e0;">
                <div style="font-size:0.7rem; color:#666;"><i class="fas fa-user-check" style="color:#16a085;"></i> Pagador de Flete</div>
                <div style="font-weight:bold; font-size:0.9rem; color:#1a5276;"><?= htmlspecialchars($viaje['pagador_nombre'] ?? '-') ?></div>
            </div>
            <div style="display:grid; grid-template-columns: 1fr 1fr; gap:6px;">
                <div style="background:#e8f5e9; border-radius:8px; padding:8px; text-align:center; border:1px solid #c8e6c9;">
                    ...
                    <div style="font-weight:bold; font-size:1rem; color:#2e7d32;"><?= number_format((float)($viaje['peso_estimado'] ?? $viaje['peso_bruto']), 1, ',', '.') ?></div>
                </div>
                <div style="background:#e3f2fd; border-radius:8px; padding:8px; text-align:center; border:1px solid #bbdefb;">
                    ...
                    <div style="font-weight:bold; font-size:1rem; color:#1565c0;">$<?= number_format((float)$viaje['tarifa_tonelada'], 0, ',', '.') ?></div>
                </div>
            </div>
            <div style="margin-top:8px; background:linear-gradient(135deg, #1b5e20, #2e7d32); border-radius:10px; padding:10px 14px; text-align:center;">
                <div style="font-size:0.65rem; color:rgba(255,255,255,0.7); text-transform:uppercase; letter-spacing:1px;"><i class="fas fa-calculator"></i> Total Flete Neto</div>
                <div style="font-size:1.4rem; font-weight:bold; color:#fff;">$ <?= number_format((float)$viaje['total_flete_neto'], 2, ',', '.') ?></div>
            </div>
            ...
            ?>
                <div style="margin-top:8px; background:#fff; border-radius:10px; padding:10px 14px; text-align:center; border:1px solid rgba(0,0,0,0.08);">
                    <div style="font-size:0.65rem; color:#666; text-transform:uppercase; letter-spacing:1px;"><i class="fas fa-weight"></i> Diferencia TN</div>
                    <div style="font-size:1.25rem; font-weight:bold; color:<?= $color ?>;"><?= $signo ?><?= number_format($abs, 2, ',', '.') ?> TN</div>
                    <div style="font-size:0.75rem; color:#777; margin-top:2px;">
                        Est: <?= number_format((float)$peso_estimado_tn, 2, ',', '.') ?> TN → Desc: <?= number_format((float)$viaje['peso_neto'], 2, ',', '.') ?> TN
                    </div>
                </div>
            <?php endif; ?>
...
<div class="card" style="margin-bottom:20px;">
    ...
    <div style="display:grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap:16px;">
        <div><strong>CTG N°:</strong><br><span><?= htmlspecialchars($viaje['ctg_nro'] ?? '-') ?></span></div>
        <div><strong>Carta Porte N°:</strong><br><span><?= htmlspecialchars($viaje['carta_porte_nro'] ?? '-') ?></span></div>
        <div><strong>Otros Docs:</strong><br><span><?= htmlspecialchars($viaje['otros_docs'] ?? '-') ?></span></div>
        <div><strong>Comisión:</strong><br><span><?= htmlspecialchars(ucfirst(str_replace('_', ' ', $viaje['comision_tipo'] ?? 'ninguna'))) ?></span></div>
        <div><strong>Valor Comisión:</strong><br><span>$ <?= number_format((float)$viaje['comision_valor'], 2, ',', '.') ?></span></div>
        <div><strong>Comisionista:</strong><br><span><?= htmlspecialchars($viaje['comisionista_nombre'] ?? '-') ?></span></div>
    </div>
</div>
    ...
    <div class="modal-content" style="max-width:450px;">
        ...
        <form method="POST">
            ...
            <div class="modal-footer">
                ...
                <button type="submit" class="btn-primary" style="background:#e67e22;"><i class="fas fa-hand-holding-usd"></i> Registrar Adelanto</button>
            </div>
        </form>
    </div>
</div>
    ...
    <div class="modal-content" style="max-width:450px;">
        ...
        <form method="POST">
            ...
            <div class="modal-footer">
                ...
                <button type="submit" class="btn-primary" style="background:#9b59b6;"><i class="fas fa-file-invoice"></i> Facturar</button>
            </div>
        </form>
    </div>
</div>
    ...
    <div class="modal-content" style="max-width:500px;">
        ...
        <form method="POST">
            ...
            <div class="modal-footer">
                ...
                <button type="submit" class="btn-primary" style="background:#27ae60;"><i class="fas fa-check-double"></i> Confirmar Cobro</button>
            </div>
        </form>
    </div>
</div>
